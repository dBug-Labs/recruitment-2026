...
/**
 * Payload for POST /api/admin/extensions — one extension applied to every
 * unsubmitted candidate inside a "how much time is left" window.
 *
 * This one takes a *length* rather than a date. A shared absolute deadline
 * would quietly shorten the window for anyone whose own deadline was already
 * later than it, which is the opposite of what "give them more time" means;
 * counting days per candidate can only ever add time.
 */
export const BulkExtendSchema = z.object({
  days: z.coerce
    .number()
    .int('Give a whole number of days')
    .min(1, 'Extend by at least one day')
    .max(60, 'That is more than 60 days — pick a shorter extension'),
  /** Narrow to a single task; omitted means every task in view. */
  taskId:      z.string().min(1).optional(),
  /** Narrow to a single domain; omitted means every domain the user may touch. */
  domain:      z.enum(ALL_DOMAIN_KEYS).optional(),
  /**
   * Only candidates with less than this many hours left. Counted forward from
   * now, so a window already contains the overdue — someone days past their
   * deadline has well under 24 hours left. 0 is the overdue on their own;
   * omitted drops the filter and takes everyone who has not submitted.
   */
  withinHours: z.coerce
    .number()
    .int('Give a whole number of hours')
    .min(0, 'Hours cannot be negative')
    .max(720, 'That is more than 30 days out — leave it off to take everyone')
    .optional(),
  /** Email everyone the new date. Off leaves it to the dashboard. */
  notify:      z.boolean().optional().default(true),
  reason:      z.string().max(200, 'Reason must be at most 200 characters').optional(),
  /** Count who would be affected and change nothing. */
