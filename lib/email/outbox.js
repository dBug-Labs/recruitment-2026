 ...
 *   EMAIL_SEND_MODE=sync    send inline and surface errors (scripts)
...
const isSyncMode      = () => process.env.EMAIL_SEND_MODE === 'sync'
...
/**
 * Hands a rendered message to the queue.
 *
 ...
 *   otherwise     — write the row, then either send it inline (`immediate`, or
 *                   EMAIL_SEND_MODE=sync) or leave it to the background pump.
 *
 * @param {{ to: string, subject: string, html: string, text: string }} payload
 * @param {{ kind?: string, priority?: number, retry?: boolean, immediate?: boolean }} [options]
 */
export async function dispatch(payload, options = {}) {
  ...
  // OTPs and script sends want the answer now: a queued OTP is a useless OTP,
  // and a script that returns before sending would exit mid-flight.
  if (options.immediate || isSyncMode()) return drainOne(id)

  // Everything else is durable already — let the pump carry it.
  pump()
