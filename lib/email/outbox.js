 ...
 * concurrent callers share the one in-flight run and ignore these options.
 *
 * ─── Why a caller would bound it ─────────────────────────────────────────────
 *
 * Sends are spaced ~900ms apart, so a queue of any size outlives a serverless
 * request. Being killed mid-`deliverRecord` is the failure this whole module
 * exists to prevent: the row sits in `sending` and nothing retries it until
 * `reclaimStaleClaims` gets to it ten minutes later. So a caller under a wall
 * clock passes `deadline` and gets `stopped: 'deadline'` back, leaving the rest
 * pending and honestly reported instead of half-sent.
 *
 * @param {{ deadline?: number, max?: number }} [options]
 *   deadline — epoch ms to stop claiming new messages at. The send already in
 *              flight is always allowed to finish.
 *   max      — stop after this many delivery attempts.
 * @returns {Promise<{ sent: number, failed: number, deferred: number,
 *                     stopped: string|null }>}
 */
export function pump(options = {}) {
  if (pumping) return pumping

  const { deadline = null, max = Infinity } = options
  ...
  pumping = (async () => {
    ...
    try {
      ...
      for (;;) {
        // Checked before claiming, never mid-send — a claimed row must always
        // reach a terminal state in the same pass that took it.
        if (stats.sent + stats.failed >= max) {
          stats.stopped = 'limit'
          break
        }
 ...
 * @param {{ deadline?: number, max?: number }} [options] — see `pump`.
 */
