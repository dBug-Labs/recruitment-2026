...
export async function sendDeadlineExtended({ name, email, taskTitle, dueAt, dashboardUrl, reason }) {
  ...
  const html = baseHtml(subject, `
    ...
    <p style="margin-top:20px"><strong>dBug Labs Team</strong></p>
  `)
  return sendEmail({ to: email, subject, html, text }, { kind: 'deadline-extended' })
}
...
export async function sendOtp({ email, otp }) {
  ...
  `)
  // Sent inline rather than queued — a code that turns up after its ten minutes
  // is worse than none, and the caller needs a real failure to show the user.
  // Capped below the reserve so OTP traffic can never starve confirmations.
  return sendEmail({ to: email, subject, html, text }, {
    kind: 'otp', priority: PRIORITY.LOW, retry: false, immediate: true,
  })
}
