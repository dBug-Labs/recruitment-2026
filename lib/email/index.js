...
/**
 * The group link on its own, for candidates whose invitation predates it.
 *
 * A second copy of the full invitation would read as a duplicate and invite
 * exactly the wrong question — "has my slot changed?" — so this repeats the
 * slot only as confirmation and asks for one thing. Also the mail to send if
 * the WhatsApp invite is ever rotated mid-drive.
 *
 * `slotAt` is optional: someone moved to interview_scheduled by hand has no
 * slot yet, and the line is dropped rather than guessed at.
 *
 * @param {{ name: string, email: string, slotAt?: Date|string }} data
 */
export async function sendInterviewGroupInvite({ name, email, slotAt }) {
  const subject = 'Join the interviews group — dBug Labs Recruitment 2026'
  const slotStr = slotAt
    ? new Date(slotAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'full', timeStyle: 'short' })
    : null
  const text = `Hi ${name},\n\n`
    + (slotStr ? `Your interview is confirmed for ${slotStr}.\n\n` : '')
    + `One thing we left out of the last email: please join the interviews WhatsApp`
    + ` group. Slot changes and last-minute instructions go out there.\n${INTERVIEW_GROUP_URL}\n`
    + (slotStr ? `\nNothing about your slot has changed.\n` : '')
    + `\ndBug Labs Team`
  const html = baseHtml(subject, `
    <h2>Join the interviews group</h2>
    <p>Hi <strong>${escapeHtml(name)}</strong>,</p>
    ${slotStr ? `<p>Your interview is confirmed for <strong>${slotStr}</strong>.</p>` : ''}
    <p>One thing we left out of the last email: please join the interviews WhatsApp group.
       Slot changes and the last-minute instructions before your round go out there.</p>
    <div style="margin:24px 0;padding:16px 18px;border:1px solid #3d1f4a;border-radius:8px;background:#160c1b">
      <a href="${INTERVIEW_GROUP_URL}" class="btn" style="margin-top:0">Join the WhatsApp group →</a>
      <div style="font-size:12px;color:#7a6d7e;margin-top:12px;word-break:break-all">
        ${INTERVIEW_GROUP_URL}
    ...
    ${slotStr ? '<p style="color:#8d8091;font-size:13.5px">Nothing about your slot has changed.</p>' : ''}
    <p style="margin-top:20px"><strong>dBug Labs Team</strong></p>
  `)
  return sendEmail({ to: email, subject, html, text }, { kind: 'interview' })
}
